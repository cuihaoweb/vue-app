import qs from 'qs';
import {PageCommon, PostPageCommon,UserInfo} from '@/common/interface';
...
// 确认修改用户名
export const updateUsername = (data: {username: string}) => axios.get('/public/resetEmail?' + qs.stringify(data));
...
// 获取用户最近的发表文章列表
export const getPostPublic = (data: PostPageCommon) => axios.get('/public/latestPost?' + qs.stringify(data));

// 删除指定文章列表
export const deletePostByUid = (data: {
  tid: string;
...
// 获取用户最近评论列表
export const getCommentList = (data: PostPageCommon) => axios.get('/public/latestComment?' + qs.stringify(data));
